angular.module('cart.service', [])
    .factory('Cart', ['$timeout', '$q', '$http', '$window', '$ionicLoading', '$rootScope', '$ionicPopup', '$ionicPlatform', 'Setting', 'Account',
        function($timeout, $q, $http, $window, $ionicLoading, $rootScope, $ionicPopup, $ionicPlatform, Setting, Account) {
            ...
            return {
                ...
                readyBuy: function(_scope, checkUrl) {
                    ...
                    try {
                            ...
                            if (singed) {
                                ...
                                Account.takeAddrs().then(function(data) {
                                    ...
                                    if (data.state == "ok") {
                                        ...
                                        Setting.prompt("请先添加收货人");
                                    }
                                }, function(resp) {
                                    defer.reject(false);
                                    Setting.prompt("获取收货人失败");
                                }).finally(function() {
                                    $ionicLoading.hide();
                                });
                        ...
                        _scope.buy = function(orders, total, orderNum) {
                            ...
                            //如果已登陆
                            if (!Account.signined) {
                                Setting.prompt('您还没有登陆哦');
                            } else {
                                _scope.order_num = orderNum;
                                _scope.getTakePeople().then(function() {
                                    if (_scope.config.checkNum > 0) {
                                        Setting.openModal('templates/pay.html', _scope, "slideInUp").then(function(payModal) {
                                            payModal.show();
                                            ...
                                            _scope.chooseAddrs = function() {
                                                Setting.openModal('templates/chooseAddress.html', _scope, "fadeInRight").then(function(caModal) {
                                                    caModal.show();
                                            ...
                                            _scope.choosePaywhy = function() {
                                                Setting.openModal("templates/payment.html", _scope, "fadeInRight").then(function(pwModal) {
                                                    pwModal.show();
                 ...
                 * @return {[type]}         [description]
                 */
                takePd: function (orderid, pdid) {
                    var defer = $q.defer()
                                    , sender = {
                                        order_id: orderid,
                                        pd_id: pdid
                                    };
                    $http.jsonp(s.getUrl('mbop_takeOrderProduct'), s.handleParams(sender)).then(function(resp) {
                        if(resp.data) {
                            if(resp.data.state == 'ok') {
                                defer.resolve(true);
                            } else {
                ...
                delOrder: function(orderNum) {
                    ...
                    $http.jsonp(s.getUrl('mbop_delOrder'), s.handleParams(sender)).then(function(resp) {
                        if(resp.data) {
                            if(resp.data.state == "ok") {
                                defer.resolve(true);
                            } else {
                                defer.resolve(false);
                                Setting.prompt("删除订单失败");
                            }
                        }
                    }, function(resp) {
                        Setting.prompt("error");
                ...
                alipay: function(arg) {
                    ...
                    $window.Alipay.Base.pay(arg, function(successResults) {
                        ...
                        if (payres === "9000") {
                            defer.resolve(true);
                            Setting.prompt("订单支付成功");
                        } else if(payres == "6001") {
                            Setting.prompt("已取消支付");
                            defer.resolve(false);
                        } else if(payres == "6002") {
                            Setting.prompt("网络连接出错");
                            defer.resolve(false);
                        } else if(payres == "4000") {
                            Setting.prompt("订单支付失败");
                            defer.resolve(false);
                        } else if(payres == "8000") {
                            defer.resolve(true);
                            Setting.prompt("正在处理中");
                        }
                    }, function(errorResult) {
                        Setting.prompt("发起支付失败");
